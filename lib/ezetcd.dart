...
class SetCondition {
  ...
  const SetCondition._();
  
  static const SetCondition PREVIOUS_EXISTS = const SetCondition._();
  static const SetCondition PREVIOUS_INDEX_EQUALS = const SetCondition._();
  static const SetCondition PREVIOUS_VALUE_EQUALS = const SetCondition._();
  
}

String _lookupSetCondition(SetCondition condition){
  switch(condition){
    case SetCondition.PREVIOUS_EXISTS:
      return 'prevExist';
    case SetCondition.PREVIOUS_INDEX_EQUALS:
      return 'prevIndex';
    default:
      return 'prevValue';
  }
...
class EtcdClient {
  ...
  Future<NodeEvent> setNode(String path, {dynamic value, Duration ttl, bool hidden, bool directory}) {
    _assertOpen();
    var completer = new Completer();
    _put(path, value:value, ttl:ttl, options: {
      'dir': directory,
      'hidden': hidden
    }).then((result) {
      if (result['errorCode'] == null) {
  ...
  /**
    * Set the node at path only if the [condition] is met.
    * 
    * Returns a [Future] which completes with the [NodeEvent] generated by setting the node at [path] with the options specified
    * if and only if the specified [condition] is met. 
    * 
    * * [value] is ignored if directory is true
    * 
    * TODO: Document nuanced behaviors
    */
  Future<NodeEvent> compareAndSetNode(String path, SetCondition condition, dynamic conditionValue, {newValue, ttl, hidden, directory}){
    var completer = new Completer();
    var options =  {
                    'dir': directory,
                    'hidden': hidden };
   var conditionName = _lookupSetCondition(condition);
   options[conditionName] = conditionValue;
       _put(path, value: newValue, ttl: ttl, options:options
       ).then((result) {
         if (result['errorCode'] == null) {
  ...
  /**
   * Delete the [Node] at [path] only if the condition is met.
   * 
   * Returns a [Future] which completes with the [NodeEvent] generated as the
   * result of deleting the node at [path].  The node is deleted if and ony if
   * [condition] is met.
   * 
   *  * [recursive] must be true to delete directories with children
   */
  ...
  Future<Map> _put(String key, {String value, Duration ttl, Map options: const {}}) {
    var completer = new Completer();
    _client.put(_host, _port, _buildUrl(key, options)).then((request) {
      request.headers.contentType = ContentType.parse('application/x-www-form-urlencoded');
      request.headers.set('accept', '*/*');
      var putContent = _encodePutContent(value, ttl);     

      request.contentLength = putContent.length;
      request.write(putContent);
  ...
  static Future _get(String key, HttpClient client, String host, int port, {Map options: const {}}) {
    var completer = new Completer();
    client.get(host, port, _buildUrl(key, options)).then((request) {
  ...
  Future _delete(String key, {Map options: const {}}) {
    var completer = new Completer();
    _client.delete(_host, _port, _buildUrl(key, options)).then((request) {
  ...
  static String _buildUrl(String key, Map options){
    var buffer = new StringBuffer();
    buffer.write('/v2/keys');
    buffer.write(key);
    buffer.write('?');
  ...
  String _encodePutContent(String value, Duration ttl){
    var buffer = new StringBuffer();
    if(value != null){
      buffer.write('&');
      buffer.write('value=');
